        ...
        for(band=0; band<HYBRID_BANDS; band++){
            if(pData->freqVector[band] < anaLim){
                ...
                formulate_M_and_Cr_cmplx(pData->hCdf, (float_complex*)pData->Cambi[band], (float_complex*)pData->Cy[band], (float_complex*)eye2,
                                            0, 0.2f, (float_complex*)pData->new_M[band], (float*)Cr);
                formulate_M_and_Cr(pData->hCdf_res, (float*)diag_Cambi, (float*)Cr, (float*)real_eye2,
                                            0, 0.2f, (float*)pData->new_Mr[band], (float*)dummy);
#else
                formulate_M_and_Cr_cmplx(pData->hCdf, (float_complex*)pData->Cambi[band], (float_complex*)pData->Cy[band], (float_complex*)eye2,
                                            1, 0.2f, (float_complex*)pData->new_M[band], (float*)dummy);
#endif
            }
            else{
                Ex = Eambi = 0.0f;
